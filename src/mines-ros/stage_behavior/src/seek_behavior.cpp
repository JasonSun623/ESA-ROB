...
void cbEsc(const std_msgs::String::ConstPtr &msg) {
    auto lastGoal = g_goals.back();
    g_goals.clear();
    g_goals.push_back(makePoseStamped(g_currentPose.position.x, g_currentPose.position.y, g_currentPose.position.z,
                                      g_currentPose.orientation.x, g_currentPose.orientation.y, g_currentPose.orientation.z, g_currentPose.orientation.w));
    g_goals.push_back(lastGoal);
    ROS_INFO("Updated path");
}
...
void moveToNextPosition() {
	...
	ROS_INFO("turnSpeed: %lf", turnSpeed);
...
int main(int argc, char **argv) {
	...
	ros::Subscriber subGoal = n.subscribe("/move_base_simple/goal", 10, cbGoal);
    ros::Subscriber escTrig = n.subscribe("esc_trig", 10, cbEsc);
	g_velPublisher = n.advertise<geometry_msgs::Twist>("cmd_vel", 10);
	ros::Rate rate(10);
